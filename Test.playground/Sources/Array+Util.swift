...
extension Array {
    ...
    // 利用闭包功能 给数组添加 包涵方法
    public func contains(includeElement: (T) -> Bool) -> Bool {
        for item in self where includeElement(item) {
            return true
        }
        return false
    ...
    public func find(includeElement: (T) -> Bool) -> T? {
        for item in self where includeElement(item) {
            return item
        }
        return nil
    ...
    public func indexOf(includeElement: (T) -> Bool) -> Int {
    ...
    public func valuesFor<U>(includeElement: (T) -> U) -> [U] {
        var result:[U] = []
        for item:T in self {

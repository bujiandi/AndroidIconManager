...
class SQLite {
    ...
    func open() -> (SQLiteHandle!,Error) {
        ...
        let dirPath = (path as NSString).stringByDeletingLastPathComponent
        if !NSFileManager.defaultManager().fileExistsAtPath(dirPath, isDirectory: &isDir) || isDir {
            do {
                ...
                return (nil, .Error(code: Int(error.code), content: error.description, userInfo: path))
            }
...
extension SQLite.Handle : SQLiteBase {
    ...
    private func execSQL(sql:String) -> Error {
        if SQLITE_OK != sqlite3_exec(_handle,sql,nil,nil,nil) {
            print(lastError)
            return lastError
        }
        return .OK
    ...
    func querySQL(sql:String) -> SQLiteResultSet? {
        ...
        if SQLITE_OK != sqlite3_prepare_v2(_handle, _lastSQL, -1, &stmt, nil) {
    ...
    func bindValue<T>(columnValue:T?,index:Int) -> CInt {
        ...
        if let v = columnValue {
            ...
            case let value as NSString:
                return sqlite3_bind_text(_stmt,CInt(index),value.UTF8String,-1,nil)
            ...
            default:
                ...
                return sqlite3_bind_text(_stmt,CInt(index),"\(v)",-1,nil)
            }
        } else {
            return sqlite3_bind_null(_stmt,CInt(index))
        }
